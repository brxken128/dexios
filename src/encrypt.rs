...
pub fn encrypt_file(input: &str, output: &str, keyfile: &str) -> Result<()> {
    ...
    let encrypted_bytes = cipher.encrypt(nonce, data.as_slice()).unwrap();
    ...
    let writer = File::create(output)?; // add error handling (e.g. can't create file)
    serde_json::to_writer(&writer, &data)?; // error = can't write to file
