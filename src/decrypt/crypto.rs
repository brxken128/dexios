...
pub fn get_key(raw_key: Vec<u8>, salt: Vec<u8>) -> [u8; 32] {
    let mut key = [0u8; 32];
...
pub fn get_information(data: &DexiosFile) -> Result<(Vec<u8>, Vec<u8>)> {
...
pub fn decrypt_bytes(data: DexiosFile, key: [u8; 32], nonce_bytes: Vec<u8>) -> Result<Vec<u8>> { // change data to the actual bytes, not the whole struct
